...
5. **Alert → ticket pipeline (one-click provisioning):**
   ...
   **Integration enabled:** Tactical runs only when API credentials are configured and `tactical_enabled` is exactly `1` (a missing setting defaults to `1`). Switching it off stops operational reads, actions, syncs and AI/MCP publication. Test Connection remains available with configured credentials. Pending webhook events are skipped, not paused or replayed. Approved offline actions stay queued without execution or reconnect-sweep dispatch; their existing expiry and reconfirmation still apply, including the expiry-only scheduled sweep while disabled.

   **Deprovision / manual cleanup:** if you disable the Tactical integration, the URLAction and AlertTemplate created by provisioning remain in Tactical. They will continue firing webhook POST requests to PSA (with a configured webhook key, PSA authenticates and persists them, then records pending events as skipped with reason `Tactical integration is disabled`; an absent webhook key is rejected with HTTP 401). To fully clean up, go to Tactical → Settings → URL Actions and delete "PSA Ticket Webhook", and Tactical → Settings → Alert Templates and delete "PSA Auto-Ticket". If PSA's template was your global default, set a new default (or clear it) in Tactical → Settings → Core Settings.
...
**Password reset request claims (B2):** This is the first migration in the cooldown-removal series. Before enabling this code, run the normal deployment migrations and verify `password_reset_claims_target_unique` exists on `password_reset_claims`, covering `(client_id, person_id)` uniquely. The claim is an atomic unique-key insert at direct execution and held approval; staging does not claim. It has no expiry or reaper. A definite CIPP answer releases it; transport exceptions, 5xx and unrecognised answers retain it. It measures the request window, not eventual Microsoft/on-prem password writeback.

If a refusal names a retained claim, **check CIPP logs for that user first** and establish that no reset request remains in progress. An authorised shell operator can then run `php artisan cipp:clear-reset-claim <claim-id> --operator='<your name>' --reason='<reconciliation reason>' --checked-cipp-log`. The operator label is the shell operator's attestation, not web authentication. Do not put a password in either option. The command records the claim id, original holder/start, operator and reason in the MCP audit log and clears that exact id in one transaction. Never clear merely because time passed: clearing an active request permits another mint. Do not roll back/drop this table with held claims without operator reconciliation; reverting application code alone reinstates the old timer, not in-flight exclusion.
...
is refused at staging with an audited reason and is pointed at the button, and is not
shown the verb in `tools/list` at all (B4.2, #2056). Say it plainly: token-lane staging
...
2. **The grant.** Choose ONE lane for the first client. For the agent lane, grant
   ...
   **Deny and re-stage anything staged before B4.1 first.** On an instance where the verb
   was ever granted before the B4.1 release, a token-lane proposal staged then carries no
   staging token in its payload; approval refuses it ("names no staging token") and it
   stays waiting forever. Before granting, an Admin opens the AI Technician cockpit,
   denies every waiting `controld_stage_onboard_client` proposal that names no stager,
