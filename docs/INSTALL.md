...
**macOS check coverage (`tactical:provision-macos-check`):** Tactical macOS/Linux agents support **script checks only** — a Windows-only script attached to a Mac (directly or via a mixed-fleet policy) fails on 100% of runs forever, leaving the Mac *visible in RMM but verified by nothing*. PSA ships a curated darwin-native health check (`resources/tactical/checks/psa-macos-health-check.sh`: data-volume disk usage fails at ≥90% used or <10 GB free; uptime/load/SIP/FileVault/OS version are report-only) and an idempotent provisioning command:

```bash
php artisan tactical:provision-macos-check                 # dry-run: prints the plan, changes nothing
php artisan tactical:provision-macos-check --apply         # upsert the script + create checks on all synced darwin agents
php artisan tactical:provision-macos-check --client-id=12 --apply    # one client
php artisan tactical:provision-macos-check --hostname=MAC-01 --apply # one agent (must be macOS)
```

Targets come from the synced `tactical_assets` snapshot (run `tactical:sync-devices` first). The script is upserted by name ("PSA macOS Health Check"), agents already carrying the check are skipped, non-darwin agents are never touched, and the command is operator-run only (not scheduled). Applying requires the API key's role to include script + check **write** permissions (Manage Scripts / Manage Checks); like alert provisioning, you can grant-then-revoke. Related guardrails: the `tactical_create_check` MCP tool refuses to attach a provably platform-incompatible script to an agent, and every Tactical device payload carries `checks_coverage` (`verified` / `unverified` / `none` / `unknown`) so a device with zero checks or only-failing checks reads as **unmonitored**, not healthy.
