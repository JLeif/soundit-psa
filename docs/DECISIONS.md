...
Source: issue #3695 and card `K5qwIx3B`, landed as PR #3696; corrected by issues #3697 and #3703. This addendum was recorded after the 2026-09-23 ruling and is not part of the record adopted under `Ujy42wr5`. It adds no obligation and amends no adopted wording. It records what the adopted clause "Logging controls cover every exposed level and generic log calls, not just the expected record's level" already reaches in the framework pinned below: a control that misses any path listed here does not cover every exposed level and generic log calls. [STANDARDS G-14](../STANDARDS.md#later-development-obligations) indexes the adopted clause and points here for what it reaches.

Laravel's logger has two level-first generic entries: `Log::log($level, $message, $context)` and `Log::write($level, $message, $context)`. Every other facade method whose return value can log also escapes a facade expectation, because its records go through the returned object and not through the facade. At `38ff0c83` the `Log` facade docblock lists these methods. The logger-returning ones are `channel()` (whose body is `return $this->driver($channel)`), `driver()`, `stack()`, `build()`, `withContext()`, `when()`/`unless()` and `getLogger()` (the underlying Monolog logger). `getChannels()` returns the resolved loggers. The manager-returning ones are `shareContext()`, `withoutContext()`, `flushSharedContext()`, `extend()` and `setApplication()`, and the manager they return logs like the facade. Controls must name both generic entries and must see or refuse every one of these methods; `getLogger()` can only be refused. No existing control is recorded as meeting this: `HdbConnectionTestActionTest::test_an_ip_filtered_refusal_audits_its_own_symbol_and_no_vendor_text` refuses `write`, `channel` and `stack` but none of the others.

A `MessageLogged` listener (`Log::listen`) receives what `Logger::writeLog` writes. That covers the facade's level methods, `log`, `write`, and records from the loggers or manager returned by the methods above. `getLogger()` is the exception: its Monolog logger fires no event. The listener has two further limits. First, `Logger::writeLog` returns before `fireLogEvent` when the channel's handler does not handle the level (`isHandling`). A record below a channel's minimum level therefore reaches no listener and no handler, for example on the `slack` channel in `config/logging.php`, which defaults to `critical`, or on any channel under a raised `LOG_LEVEL`. A listener control sees only the levels the test environment's channels handle. Second, the event carries the level string exactly as the caller passed it, so `Log::log('WARNING', ...)` arrives as `'WARNING'`, and a listener that compares against `'warning'` misses it.

Measured by execution on 2026-09-25 under `Log::spy()`. That run recorded no commit or package versions, and its probe was not kept. The probe booted the application, called `Log::spy()`, made the call under test, then ran the expectation. Results: `Log::write('warning', ...)` passes `shouldNotHaveReceived` for all eight levels and `log`, so an exclusion control that omits `write` reads green. `Log::channel(...)->warning(...)` makes the spied `channel()` return null, and the production call then errors. A generic `Log::log('warning', ...)` does not satisfy a per-level `warning` expectation (Mockery raises `InvalidCountException`). A per-level closure registered on `log` and expecting `($message, $context)` binds `$message = 'warning'` and `$context` = the sentence. What that does depends on the closure. If it filters on `$message`, it drops every generic record: an absence control then passes green, and a presence control expecting one record fails red. If it indexes `$context` by key, PHP raises a `TypeError`. The dangerous case is the green absence control.

Re-derived from source on 2026-09-26, not re-executed, at `38ff0c83` with laravel/framework v12.69.2 and mockery/mockery 1.6.12 (both pinned in `composer.lock`). `LogManager` declares no `write`, so the facade's `write` reaches the spy as a call named `write`, and no `shouldNotHaveReceived` over the eight levels and `log` inspects that call. A spy returns null for any call it has no expectation for unless the method declares a return type (`Mockery\Mock::__call`). None of the methods listed above declares one on `LogManager`, and several exist only through its `__call`. So on a spied facade each of them returns null, and a log call chained onto it errors. The listener limits above follow from `Logger::writeLog` and `Logger::fireLogEvent`. These facts depend on the installed versions: after any laravel/framework or mockery upgrade, re-run the probe at the new commit before relying on them.
