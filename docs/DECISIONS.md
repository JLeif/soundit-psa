...
Source: issue #3695 and card `K5qwIx3B`, landed as PR #3696; corrected by issues #3697 and #3703. This addendum was measured after the 2026-09-23 ruling and is not part of the record adopted under `Ujy42wr5`; it sharpens that ruling's "generic log calls" clause.

Laravel's logger has two level-first generic entries: `Log::log($level, $message, $context)` and `Log::write($level, $message, $context)`. `Log::channel()` and `Log::stack()` return a different logger, so a facade expectation never sees their records. Controls must name both generic entries and must see or refuse `channel()`/`stack()`, as some existing controls already do. A `MessageLogged` listener (`Log::listen`) receives records from all of these entries with their real level.

Measured by execution on 2026-09-25 under `Log::spy()`: `Log::write('warning', ...)` passes `shouldNotHaveReceived` for all eight levels and `log`, so an exclusion control that omits `write` reads green. `Log::channel(...)->warning(...)` makes the spied `channel()` return null, and the production call then errors. A generic `Log::log('warning', ...)` does not satisfy a per-level `warning` expectation (Mockery raises `InvalidCountException`). A per-level closure registered on `log` and expecting `($message, $context)` binds `$message = 'warning'` and `$context` = the sentence. What that does depends on the closure. If it filters on `$message`, it drops every generic record: an absence control then passes green, and a presence control expecting one record fails red. If it indexes `$context` by key, PHP raises a `TypeError`. The dangerous case is the green absence control.
