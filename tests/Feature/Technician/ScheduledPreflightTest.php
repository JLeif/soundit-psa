...
class ScheduledPreflightTest extends TestCase
{
    ...
    private function insertRow(int $runId, string $state): void
    {
        DB::table('scheduled_authorizations')->insert([
    ...
    {
        for ($i = 1; $i <= 151; $i++) {
            $this->insertRow($i, $i <= 150 ? 'dispatch_intent' : 'private-unknown-state');
        }
        $this->clock(true);
        $this->assertSame(1, Artisan::call('technician:scheduled-preflight'));
        $text = Artisan::output();
    ...
    public function test_completed_is_a_known_terminal_state_and_never_counts_as_unknown_or_pending(): void
    {
        $this->insertRow(1, 'completed');
        $this->clock(true);
        $this->assertSame(0, Artisan::call('technician:scheduled-preflight'));
        $output = json_decode(Artisan::output(), true, 512, JSON_THROW_ON_ERROR);
        $this->assertSame(1, $output['inventory']['completed']);
