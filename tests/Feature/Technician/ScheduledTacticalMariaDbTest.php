...
class ScheduledTacticalMariaDbTest extends ScheduledTacticalTest
{
    ...
    {
        $this->proposal('tactical_stage_maintenance', ['enabled' => true]);
        $id = $this->admit();
        $go = dirname(getenv('SCHEDULED_TEST_SOCKET')).'/tactical-go-'.bin2hex(random_bytes(6));
        $posts = $go.'.posts';
