...
- G-14: Operator-facing runtime strings (including log context, exception messages and operator output) that assert a mechanism — why something happened, how a function behaves, or what is or is not true of data — must be true on every emitting path. Pure event labels, which name an event without asserting a mechanism, are outside this rule. Work one site at a time: (1) delete the explanatory clause when structured keys already carry the fact or the clause is false or unverifiable on any emitting path; (2) otherwise assert the claim on every emitting arm, killing a swap/merge-arms mutant or, for a single arm, a predicate-weakening mutant; controls must cover every level the logger exposes and both level-first generic entries, `Log::log($level, $message, $context)` and `Log::write($level, $message, $context)`, by name, and must see or refuse `Log::channel()`/`Log::stack()`, whose loggers bypass facade expectations (a `MessageLogged` listener sees all of them with their real level), since counting one level cannot exclude a second record, and a per-level closure registered on `log` receives the level word as its `$message` argument — so a closure that filters on `$message` discards every generic record (PR #3614 round-one `diff:1`); controls must also assert the level, not only capture or count records, since a control that accepts other levels without reading them cannot see the record move onto one (#3586: info/debug/error stubbed and never read, so moving the cause onto `Log::error` survived), and a capture that accepts every level and asserts none cannot see a demotion (#3599, PR #3614 round-one `contract:1`, and a pre-commit draft of the #3675 reply-threading control, a `Log::listen` capture in which a warning-to-debug demotion mutant survived); (3) use prose with an executable citation only as a last resort. Verify truth by isolated execution before pinning text; never bulk-assert strings or cite suite totals as truth evidence. Cite methods rather than unstable line numbers. Review new mechanism strings against this rule; comments and names retain the separate delete-or-rename remedy; [DECISIONS / Runtime mechanism strings](docs/DECISIONS.md#runtime-mechanism-strings).
- G-15: A seat that merges to `main` owes the release in the same sitting: where Jeeves's release go covers the merge (G-11), deploy through the normal gate and verify at the consumer boundary; without that go, or when deliberately not shipping, record the merge as unshipped — sha and reason in the no-ship acknowledgement list, and on the owning card naming who ships it. G-15 grants no release or production authority, and a deploy-*gate* PASS is neither release authority nor a deploy — ending after it leaves every checkpoint reading green while users have none of the code; [DECISIONS / Freeze, release and production authority](docs/DECISIONS.md#release-conduct).
