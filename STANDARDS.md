...
- G-14: Operator-facing runtime strings (including log context, exception messages and operator output) that assert a mechanism — why something happened, how a function behaves, or what is or is not true of data — must be true on every emitting path. Pure event labels, which name an event without asserting a mechanism, are outside this rule. Work one site at a time: (1) delete the explanatory clause when structured keys already carry the fact or the clause is false or unverifiable on any emitting path; (2) otherwise assert the claim on every emitting arm, killing a swap/merge-arms mutant or, for a single arm, a predicate-weakening mutant; controls must cover every level the logger exposes and generic log calls, not just the expected record's level (the adopted wording). The [2026-09-25 addendum](docs/DECISIONS.md#runtime-mechanism-strings-addendum) is not part of the adopted record and adds no obligation. It is not a checklist: whether a control complies is judged under the adopted wording alone. It records the following. These are inferred from source at a pinned commit, not measured. Both level-first generic entries, `Log::log` and `Log::write`, exist. A facade expectation never sees a record that the code under test writes through an object a facade method returned. Examples are `channel()`, `stack()` and Monolog's `withName()` reached through the `__call` chain, and the set is open. A control that captures or counts records without reading their level cannot see one move or be demoted. It also records the per-level-closure binding hole, the limits of a `MessageLogged` listener and the recorded instances of each hole; (3) use prose with an executable citation only as a last resort. Verify truth by isolated execution before pinning text; never bulk-assert strings or cite suite totals as truth evidence. Cite methods rather than unstable line numbers. Review new mechanism strings against this rule; comments and names retain the separate delete-or-rename remedy; [DECISIONS / Runtime mechanism strings](docs/DECISIONS.md#runtime-mechanism-strings).
