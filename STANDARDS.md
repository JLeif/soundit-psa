...
- G-15: A seat that merges to `main` owes the release in the same sitting: either deploy and verify at the consumer boundary, or record on the owning card that the merge is deliberately unshipped and name who ships it. Running the deploy *gate* is an authorization, not a deploy — ending between the two leaves every checkpoint reading green while users have none of the code; [DECISIONS / Freeze, release and production authority](docs/DECISIONS.md#release-conduct).
