...
class McpToolRegistry
{
    ...
    public static function blockCallerTool(bool $internal = false): array
    {
        return [
            ...
            'description' => "Block the caller of one phone call by adding their number to the phone directory Blocked list, reusing the native path. CLIENT-VISIBLE: future calls from that number are hung up by the IVR — this silences a real phone line. Refuses an unparseable caller number, and refuses (reporting the existing list) a number already in the directory rather than moving it between lists. Default-ungranted including legacy tokens. Bare or :staged grants hold for cockpit approval; only explicit :immediate permits immediate execution. Staged proposals revalidate the call, its caller number and the directory at approval. Audited by ids and reason only — the caller's number is never written to the action log.",
            'input_schema' => [
                'type' => 'object',
                'properties' => [
                    'phone_call_id' => ['type' => 'integer', 'minimum' => 1],
    ...
    public static function allowCallerTool(bool $internal = false): array
    {
        return [
            ...
            'description' => 'Add the caller of one phone call to the phone directory Allowed list, reusing the native path — future calls from that number ring through. THIS DOES NOT UN-BLOCK: a number already in the directory (on either list) is REPORTED with its current list and left untouched, so a block placed by someone else can never be silently lifted; removing an entry is a phone-directory action, not this tool. Refuses an unparseable caller number. Default-ungranted including legacy tokens. Bare or :staged grants hold for cockpit approval; only explicit :immediate permits immediate execution. Staged proposals revalidate the call, its caller number and the directory at approval.',
            'input_schema' => [
                'type' => 'object',
                'properties' => [
                    'phone_call_id' => ['type' => 'integer', 'minimum' => 1],
