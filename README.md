...
A Client's own `email` is a separate field from its People's emails. It is written through
`ClientService`: `createClient`/`updateClient`, reached from the Clients create/edit forms
(validated request) and from the staff MCP `create_client`/`update_client` tools; and
`mergeClients`, which fills a blank survivor Email from the merged duplicate Client. None of
these copies a Person's email onto their Client, and marking a Person **Primary** moves that
badge without changing the Client's Email. Editing a Person updates that Person's row
