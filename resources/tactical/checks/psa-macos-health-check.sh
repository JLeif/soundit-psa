#!/bin/bash
#
# PSA macOS Health Check (psa-0pb9m)
#
...
#
# Design rules:
#   - darwin-native tools only (df, sysctl, csrutil, fdesetup, sw_vers, awk) —
#     all present on every supported macOS; no Homebrew, no network calls.
...
#   - FAILS (exit 1) only on objective, actionable problems: the data volume
#     at/above 90% used, or under 10 GB free (mirrors the PSA's EndpointInsight
#     LOW_DISK thresholds). Everything else is REPORT-ONLY — a check that
#     fails on org-policy variance (e.g. FileVault intentionally off) would
#     recreate the always-failing noise this bead removes.
#   - Fast (<5s) and quiet: one "fact: value" line per signal, one verdict line.
#
# Exit codes: 0 = healthy, 1 = problem found. Tactical treats 0 as passing.
...
if [ -n "$BOOT_S" ]; then
    ...
    echo "uptime_days: ${UP_DAYS}"
fi

CORES=$(sysctl -n hw.ncpu 2>/dev/null || echo 0)
LOAD1=$(sysctl -n vm.loadavg 2>/dev/null | awk '{print $2}')
[ -n "$LOAD1" ] && echo "load_1m: ${LOAD1} (cores: ${CORES})"
...
[ -n "$FILEVAULT" ] && echo "filevault: ${FILEVAULT}"

OS_VERSION=$(sw_vers -productVersion 2>/dev/null)
[ -n "$OS_VERSION" ] && echo "macos_version: ${OS_VERSION}"

# ── Verdict ──────────────────────────────────────────────────────────────────
if [ -n "$PROBLEMS" ]; then
    echo "VERDICT: PROBLEM - ${PROBLEMS}"
    exit 1
fi
